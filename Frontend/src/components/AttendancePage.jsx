...
const AttendancePage = () => {
    ...
    const handleSubmit = async () => {
        try {
            ...
            const presentStudentRegNos = students
                .filter((student) => attendance[student.id])
                .map((student) => student.reg_no);

            console.log("Present student reg_nos:", presentStudentRegNos);

            const res = await axios.post(
                "http://localhost:5000/api/mark_attendance",
                {
                    subject_id: selectedSubject,
                    student_ids: presentStudentRegNos,
                },
                {
                    headers: {
                        Authorization: `Bearer ${token}`,
...
const ActionButton = ({ onClick, color, label }) => (
    <button
        onClick={onClick}
        className={`mt-6 w-full text-white py-2 px-4 rounded-lg transition-colors bg-${color}-600 hover:bg-${color}-700`}
    >
        {label}
    </button>
);
